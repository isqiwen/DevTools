...
from BuildSystem.ConfigLoader import ConfigLoader
...
class ProjectConfig:
    ...
    def initialize(cls, build_config_file=None):
        ...
        if build_config_file is None:
            dev_tools_root = Path(__file__).resolve().parent.parent
            build_config_file = dev_tools_root / "Config" / "ProjectConfig.json"

        build_config_file = Path(build_config_file)
